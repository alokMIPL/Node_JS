...
Now suppose we perform an operation and it is a **blocking (synchronous) operation**. In that case, the request is handled by the **thread pool**, and Node.js uses a limited number of threads (by default, **4 threads**).

If many operations arrive at the same time, all threads can become busy. As a result, new requests have to wait, which increases the waiting time and can slow down the application.

Therefore, it is better to use **asynchronous (non-blocking) operations** to improve performance and handle multiple requests efficiently.
